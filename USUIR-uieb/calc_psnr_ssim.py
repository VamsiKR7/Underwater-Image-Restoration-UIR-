...
parser.add_argument('--input_dir', type=str, default='./results/predict/J')
parser.add_argument('--refer_dir', type=str, default='../Dataset/UIE/UIEBD/test/label')
parser.add_argument('--output_dir', type=str, default='./results/predict/J')
...
assert len(input_image_paths) == len(refer_image_paths), "Number of input images and reference images must be the same"
...
for input_img_path, refer_img_path in zip(input_image_paths, refer_image_paths):
    try:
        ...
        # Always resize to ensure matching sizes
        if args.resize:
            input_img = input_img.resize((args.width, args.height))
            refer_img = refer_img.resize((args.width, args.height))
        else:
            # If not explicitly resizing, ensure they match by resizing input to match reference
            if input_img.size != refer_img.size:
                print(f"Warning: Size mismatch for {os.path.basename(input_img_path)}. Resizing input to match reference.")
                input_img = input_img.resize(refer_img.size)
        
        # Double-check sizes match
        if input_img.size != refer_img.size:
            print(f"Error: Size mismatch even after resizing for {os.path.basename(input_img_path)}. Skipping this image.")
            continue
        ...
        print(f"Error processing {os.path.basename(input_img_path)}: {str(e)}")
...
if psnr_values and ssim_values:
    ...
    f.write("average_value,0.000,0.000")
    if args.use_log:
        print("No valid image pairs were processed. Check your input and reference directories.")
